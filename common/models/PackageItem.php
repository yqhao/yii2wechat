...
//            $packageItemStock = PackageItem::find()->andWhere(['id'=>$this->package_id])->select('SUM(stock) AS total_stock')->scalar();
            $packageItemStock = Yii::$app->db->createCommand("SELECT SUM(stock) AS total_stock FROM ".PackageItem::tableName()." WHERE package_id=:id",[':id'=>$this->package_id])->queryScalar();

            $package = Package::findOne(['id'=>$this->package_id]);
            $package->stock = $packageItemStock;
            if(!$package->save()){
                throw new Exception('更新库存失败');
            }
//            Yii::$app->db->createCommand()->update(Package::tableName(),['stock'])->execute();
